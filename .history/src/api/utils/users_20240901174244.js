...
router.get('/:user', function(req, res){
   const getData = new Promise((resolve) => {
      mongoAPI.getItemData(
        { db: 'tasksDB', collection: 'users', query: { "userName": req.params.body } }
      ).then((data, err) => resolve(data))
    }).then((data) => {
      return res.json(data)
...
router.post('/login', function(req, res){
  const getData = new Promise((resolve) => {
    mongoAPI.getItemData(
     { db: 'tasksDB', collection: 'users', query: { "email": req.params.body.email, "password": req.params.body.password } }
    ).then((data, err) => resolve(data))
  }).then((data) => {
    if(data !== null) {
      data = {
